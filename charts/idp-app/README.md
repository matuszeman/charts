...
- **[Config management built in](tests/values/examples/config-management.yaml)** — the `configs` block creates ConfigMaps, Secrets, ExternalSecrets (AWS Secrets Manager), and SealedSecrets, then mounts them into containers via `configDirs`, `configFiles`, or `env` — all validated at template time.
...
- **[Automatic rolling restarts on config changes](tests/values/examples/rolling-restarts.yaml)** — set `restartPodOnUpdate: PodAnnotation` or `NameSuffix` on any config to have pods restart automatically when content changes, without manual annotation management.
